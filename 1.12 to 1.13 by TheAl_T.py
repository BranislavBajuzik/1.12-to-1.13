...
# Block data value -> block state database by: Onnowhere http://youtube.com/onnowhere2
...
def block(data, setMode, nameLabel, stateLabel=None, nbtLabel=None):
    ...
    if setMode:
        if stateLabel in data:
            ...
            if data[stateLabel] != "default" and blockStates[data[nameLabel]][0] != data[stateLabel]:
...
selectorArgs = ("x", "y", "z", "dx", "dy", "dz", "type", "lm", "l", "m", "team", "score", "name", "tag", "nbt", "rm", "r", "rxm", "rx", "rym", "ry", "c")
selectorArgsNew = (('x', 'x', 'x'), ('y', 'y', 'y'), ('z', 'z', 'z'), ("dx", "dx", "dx"), ("dy", "dy", "dy"), ("dz", "dz", "dz"), ("type", "type", "type"), ("level", 'lm', "l"), ("gamemode", 'm', 'm'), ("team", "team", "team"), ("score", "score", "score"), ("name", "name", "name"), ("tag", "tag", "tag"), ("nbt", "nbt", "nbt"), ("distance", 'rm', "r"), ('x_rotation', "rxm", "rx"), ('y_rotation', "rym", "ry"), ("limit", 'c', 'c'), ("sort", "sort", "sort"))
def futurizeSelector(data):
    ret = []
    for future, low, high in selectorArgsNew:
        if future != 'score':
            ...
            for i, word in enumerate(syntax):
                ...
                if word[1] == '@':
                    ...
                    try:
                        ...
                        if workTokens[i-1] not in blockStates:
                            raise SyntaxError(u"{} is not a valid block".format(tokens[i-1]))
                        workTokens[i] = blockStates[workTokens[i-1]][workTokens[i]] if workTokens[i] != -1 else "-1"
                    except ValueError:
                        if not blockStateRe.match(workTokens[i]):
        ...
        if raw[0] == '@':
            ...
            if len(raw) > 2:
                ...
                for key in self.data.keys():
                    if not (scoreRe.match(key) or key in selectorArgs):
...
class function(Master):
    def __init__(self, tokens):
        ...
        self.syntax, self.data = lex(self.__class__.__name__, syntaxes, tokens)
        if not functionRe.match(self.data["<.function"]):
...
class gamerule(Master):
    ...
    def __unicode__(self):
        if self.data["<.rule"] not in gamerules:
            global comentedOut
            commentedOut["gamerule"] = None
...
class help(Master):
    def __init__(self, tokens=None):
        Master.__init__(self)
        if tokens:
            syntaxes = (("<({}".format("|".join(commands + map(str, range(1, 9)))), ), )
            self.syntax, self.data = lex(self.__class__.__name__, syntaxes, tokens)
...
class stats(Master):
    def __init__(self, tokens):
        Master.__init__(self)
        self.stat = "<({}".format("|".join(statTags))
...
class toggledownfall(Master):
    ...
    def __unicode__(self):
        global comentedOut
        commentedOut["toggledownfall"] = None
...
def convertFile(name):
    ...
    try:
        ...
        print "File: {}\nLine {}:\n{}".format(name, i+1, e)
        return name
    with codecs.open(u"{}.TheAl_T".format(name), 'w', "utf-8") as f:
        f.writelines(lines)
        ...
        if sys.argv[1:]:
            ...
            if choice.strip() in ('1', '2', '3', '4'):
                ...
                if choice.strip() == '1':
                    ...
                    if not found:
                        found = 'y' == raw_input(
                            u"Unable to find the world directory. If you are sure this: \'{}\' is a world directory, press \'y\' to continue: ".format(os.getcwdu())).lower()
    ...
    if not manual:
        if failedFiles:
            ...
            for tmp in tmpFiles:
                ...
                os.rename(tmp, tmp[:-8])
            if commentedOut:
                print u"Some commands were commented out because they have to be converted manually. To find them, search for:\n\'#~ {}\' or \' ||| \'\n".format(u"\', \'#~ ".join(commentedOut.keys()))
            raw_input(u"A total of {} commands, across {} files, was converted in {} seconds\n\nPress Enter to exit...".format(commandCounter, fileCounter, getTime() - start))
    else:
