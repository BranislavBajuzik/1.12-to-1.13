        ...
        try:
            ...
            for i, word in enumerate(syntax):
                ...
                if word[1] == '@':
                    ...
                    workTokens[i-1] = noNamespace(workTokens[i-1])
                    try:
    ...
    def __unicode__(self):
        ...
        if 'm' in self.data:
            ...
            if self.data['m'] == '0' or self.data['m'] == 's':
                ...
                raise SyntaxError(u"m\'s value in \'{}\' has to be in (!|)(0|1|2|3|s|c|a|sp|survival|creative|adventure|spectator)".format(self.raw, key))
...
class scoreboard(Master):
    def __init__(self, tokens):
        ...
        syntaxes = (("<(objectives", "<(list"),
                    ...
                    ("<(players", "<(test", "<(*",      "<.objective", "<.min", "[.max"),  # todo already asked, waiting
    ...
    def __unicode__(self):
        ...
        if "<(tag" in self.data:
            if "[{dataTag" in self.data:
                newSelector = Selector(self.data["<@entity"].raw)
                newSelector.data["nbt"] = u"{}".format(self.data["[{dataTag"])
                end = -1
            else:
                newSelector = self.data["<@entity"]
                end = len(self.syntax)
            s = u"tag"
            for key in self.syntax[2:end]:
                s += u" {}".format(self.data[key] if key != "<@entity" else newSelector)
            return s
