...
# Made by TheAl_T
# HUGE thanks to Onnowhere for block state database / conversion and for testing and feedback
    ...
    def __init__(self, raw):
        ...
        for key in ("x", "y", "z", "dx", "dy", "dz", "lm", "l", "rm", "r", "rxm", "rx", "rym", "ry", "c"):
            try:
                ...
                raise SyntaxError(u"{}\'s value in \'{}\' has to be integer".format(key, self.raw))
        ...
        if 'c' in self.data:
            try:
                ...
                raise SyntaxError(u"c\'s value in \'{}\' has to be integer".format(self.raw))

        if "type" in self.data:
            if self.data["type"][0] == "!":
                if self.data["type"][1:] not in Globals.summons:
                    raise SyntaxError(u"\'{}\' is not valid entity type".format(self.data["type"][1:]))
            else:
                if self.data["type"] not in Globals.summons:
